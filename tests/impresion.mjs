...
// Se prueba con el área normal y con 8 mm menos arriba y abajo (lo que achica Safari con encabezados/pies o una impresora con márgenes grandes).
...
for (const extraMM of [0, 8]) {
  ...
  const hojas = await page.evaluate((extraMM) => {
    ...
    if (extraMM) { const st = document.createElement('style'); st.textContent = `@media print{@page{size:A4 portrait;margin:${5 + extraMM}mm 8mm}}`; document.head.appendChild(st); }
    return nh;
  }, extraMM);
  await page.emulateMedia({ media: 'print' });
  const pdf = (await page.pdf({ preferCSSPageSize: true })).toString('latin1');
  const paginas = (pdf.match(/\/Type\s*\/Page(?![s\w])/g) || []).length;
  const txt = `área útil −${2 * extraMM} mm: ${hojas} hojas armadas → ${paginas} páginas impresas`;
